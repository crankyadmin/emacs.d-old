...
** add custom path for themes that aren't installed by cask
...
#+BEGIN_SRC emacs-lisp
  (setq hrs/default-font "Office Code Pro D")
...
Turn on Ligitures, this only works with railwaycats build of Emacs
...
(defun crankyadmin-mark-sexp ()
(interactive)
(mark-sexp))

(global-set-key (kbd "C-c m r") 'crankyadmin-mark-sexp)
